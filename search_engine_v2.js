...
'use strict';
const VERSION='ldc-search-v2-engine-v1';
...
function parseQuery(raw){
  const original=String(raw||''); let q=normalise(original),volume=null,year=null,date=null,ref=null;
  ...
  for(const [month,mi] of Object.entries(MONTHS)){
    const rx=new RegExp(`\\b([0-3]?\\d)\\s+${month}\\s+((?:18|19|20)\\d{2})\\b`);const dm=q.match(rx);
    if(dm){const day=Number(dm[1]);year=Number(dm[2]);date=`${String(year).padStart(4,'0')}-${String(mi).padStart(2,'0')}-${String(day).padStart(2,'0')}`;q=(q.slice(0,dm.index)+q.slice(dm.index+dm[0].length)).replace(/\s+/g,' ').trim();break;}
  }
  if(!date){m=q.match(/\b((?:18|19|20)\d{2})\b/);if(m){year=Number(m[1]);q=(q.slice(0,m.index)+q.slice(m.index+m[0].length)).replace(/\s+/g,' ').trim();}}
  m=original.match(/\b(?:LDC|LDCSUP)\.[A-Za-z0-9._-]+/i);
  if(m){ref=m[0].toUpperCase();q=normalise(original.slice(0,m.index)+' '+original.slice(m.index+m[0].length));}
...
function createEngine(payload){
  ...
  const N=index.N,dl=decodeDocLengths(index.doc_lengths_u16le_b64,N),bits=b64bytes(jf.bits_b64),postCache=new Map(),normCache=new Map(),entryDocs=Array.from({length:entries.length},()=>[]);
  for(let i=0;i<documents.length;i++){const ei=documents[i][1];if(entryDocs[ei])entryDocs[ei].push(i);}
  ...
  function scopeAllows(i,d,p,opt){
    ...
    if(p.date&&iso!==p.date)return false;if(p.year&&!iso.startsWith(String(p.year)))return false;
  ...
  function metadataSearch(p,opt){
    ...
    for(let ei=0;ei<entries.length;ei++){const e=entries[ei];if(!entryModeAllows(e,opt.mode))continue;if(opt.volMin&&e[2]<opt.volMin)continue;if(opt.volMax&&e[2]>opt.volMax)continue;if(p.volume&&e[2]!==p.volume)continue;
      const iso=e[5]||'';if(p.date&&iso!==p.date)continue;if(p.year&&!iso.startsWith(String(p.year)))continue;
      let score=0,kind='Titre';const titleN=normalise(e[3]||''),ref=String(e[1]||'').toUpperCase();
      if(p.ref){if(ref===p.ref)score=2000;else if(ref.startsWith(p.ref))score=1800;else continue;kind='Référence';}
