...
- Three independently traceable sub-ledgers share this engineering successor: `REC-LDC-COL-01`, `PRIV-LDC-DIAG-01`, `UPD-LDC-DRAFT-01`.
- Whole-collection deletion now captures the exact parent and children deleted in the same transaction and offers a 10-second atomic Undo. Restore refuses parent/child/unexpected-orphan conflicts rather than overwriting newer durable data.
- Default copied diagnostics no longer include the raw URL query or raw exception messages and never inspect user-authored notes/highlights/collection notes/report comments/selected text. Controlled diagnostic kind/name/timestamp and public technical/corpus identifiers remain available.
- Service Worker `controllerchange` no longer reloads the page automatically. A central volatile dirty/pending-state guard blocks explicit refresh while a note, collection note, new collection name, issue-report comment, backup preview or active backup import would be interrupted.
- Degraded boot was studied and deferred; no degraded runtime is added. `CONC-LDC-MIGRATION-BYPASS-01` remains open for later migration/startup hardening before promotion.
- v81 scalable backup, v80 prepared-offline retention and v79 collection integrity remain inherited. Stage-5B backup import semantics are unchanged except for the authorized transient busy flag used by the update guard.
- `speech_model.js`, canonical/devotional wording, paragraph IDs/order, speaker/display/flow authority, SEARCH-V2 payload/ranking semantics, supplement content and DB schema remain unchanged.
- Pre-freeze exact QA: Stage-6 recovery **14/14**, privacy **12/12**, update **18/18**, inherited Stage-4A **17/17 + 6/6**, Stage-5B structural safety PASS. Final deterministic/reopened package QA is external.
- Residuals carried forward: four inherited comma-spacing corpus artifacts; ten inherited v74 checkpoint archives not carried; `CONC-LDC-MIGRATION-BYPASS-01`; and the non-safety UI wording residual that Help still labels the normal backup “Sauvegarde JSON” although v81+ exports `.ldcbackup` by default.
- Physical iPhone/iPad/Samsung, installed-PWA update/draft behavior, live-origin, true-offline, storage-pressure, large-backup restore and accessibility gates remain external.
...
> **Historical/inherited boundary:** Everything below is predecessor or inherited-authority documentation. v81 scalable backup, v80 offline retention and v79 transactional integrity remain active inherited authorities, but older CURRENT/version/deployment wording cannot override v82.

<!-- V81_CURRENT_START -->
# INHERITED ACTIVE AUTHORITY — v2.19.81-R1B BKP-LDC-01 STAGE 5B SCALABLE BACKUP/RESTORE SUCCESSOR

- Direct predecessor: immutable `v2.19.80-R1B` SHA-256 `d94da78f2f29378bb0e672d26dcc430c9303de82e35aa588d7b8001bbbb89e3b`.
- Owner authorization: exact frozen BKP-LDC-01 Stage 5B proposal SHA-256 `cee0dc13ce06dc41a32a448bcadb2b0d22f60d57e1517bba17f1acfeec1227f1`.
- Functional mutation is limited to `index.html`: DB v2→v3 adds only noncanonical `backup_stage` + `backup_stage_meta`; new normal `.ldcbackup` stream export/import; bounded parsing/staging/validation; and one same-database atomic final swap of all eight canonical stores.
- Legacy `.json` restore remains a compatibility path at the inherited 10 MiB one-pass ceiling and retains existing historical migration behavior. Historical `migrateUserData*` functions are not modified.
- New normal export reads all eight canonical stores in one readonly transaction, uses deterministic cursor order and bounded Blob/text parts, and does not `getAll()` the full personal database.
- New stream import validates into noncanonical shadow stores first. Canonical personal data is untouched until confirmation; the final replacement and shadow cleanup occur in one IndexedDB readwrite transaction, so abort preserves the old canonical state.
- v80 Stage 4B offline-retention algorithms and v79 Stage 4A collection-transaction functions are inherited unchanged.
- `speech_model.js` remains protected; canonical/devotional wording, paragraph IDs/order, speaker/display/flow authority, SEARCH-V2 payload/ranking semantics and supplement content are unchanged.
- Pre-freeze exact QA: stream parser **15/15 PASS**; IndexedDB export/swap model **5/5 PASS**; DB v2→v3 upgrade/blocked path **2/2 PASS**; ~64.835 MiB / 8,194-record bounded export PASS; inherited Stage 4A **17/17 + 6/6 PASS**. Final reopened-package QA and mutant results are external receipts written after deterministic freeze.
- Residual risks carried forward unchanged: four inherited regular-space-before-comma corpus artifacts at `LDC.T28.E0023.P070`, `LDC.T34.E0040.P016`, `LDC.T36.E0036.P021`, `LDC.T36.E0046.P016`; ten inherited v74 checkpoint source archives are not carried in this lineage; and `CONC-LDC-MIGRATION-BYPASS-01` remains a separate skipped-version/migration concurrency promotion gate.
- Physical large-state restore, storage-pressure/quota, installed-PWA v80→v81 DB upgrade, mixed-version blocked-upgrade, true-offline, live-origin and accessibility gates remain external.
- Public deployment: **NOT AUTHORIZED BY THIS PACKAGE**.
...
> **Historical/inherited boundary:** Everything below this line is predecessor or inherited-authority documentation. v80 offline retention and v79 transactional integrity remain active, but older package CURRENT/version/deployment wording cannot override v81.

<!-- V80_CURRENT_START -->
# INHERITED ACTIVE AUTHORITY — v2.19.80-R1B STAGE 4B PREPARED-OFFLINE RETENTION SUCCESSOR

- Direct predecessor: immutable `v2.19.79-R1B` SHA-256 `88f7e06831bc9cae4f69155abe8dabfe1f12a9b6b348e41648fe2b5fe4aa9930`.
- Owner authorization: exact frozen Stage 4B proposal SHA-256 `c2c20cb7051692ab0801171407448416052089573ee95b832483b805568b9c5b`.
- Functional mutation is limited to `sw.js`, strictly necessary offline-state portions of `index.html`, and `offline_manifest.json`.
- Shell/runtime caches remain app-versioned. Prepared offline data now uses storage schema `ldc-offline-storage-v3` with a physical cache name derived from service-worker scope rather than app version.
- Current offline asset identity is path + exact SHA-256 + exact byte length within the current service-worker scope. A predecessor global content binding alone does not invalidate an otherwise exact asset.
- Legacy versioned LDC offline caches are preserved at activation and may satisfy exact current assets without copying 215.9 MB into a new cache. Missing/changed/invalid assets alone are downloaded to the stable scope-specific cache.
- Offline clear is scope-safe for shared legacy caches; another service-worker scope's entries are preserved.
- Completion state is bound to storage/content identity, not a prior app-version cache name.
- v79 CONC-LDC-01 Stage 4A transactional functions and semantics are inherited unchanged.
- `speech_model.js` remains byte-identical at `3bd067956a5275e0be7231add3b425c124fb17f3d0be548080eee10c649134f6`; canonical/devotional wording, paragraph IDs/order, speaker/display/flow authority, SEARCH-V2 payload/ranking semantics, supplement content, IndexedDB schema and historical migrations remain unchanged.
- Engineering QA on the exact v80 work tree: service-worker offline suite **17/17 PASS**; offline completion-state suite **3/3 PASS**; scope/protection verifier **27/27 PASS**; independent second-path challenge **42/42 PASS**; Stage 4A inherited transaction/staleness suites **17/17 + 6/6 PASS**; deliberate Stage 4B mutants **4/4 caught**; inherited FAST fixed point **2,996 affected records + 288/288 flow joins, zero errors**.
- Managed Chromium localhost navigation was reattempted against v80 and remains environment-blocked before app execution by `ERR_BLOCKED_BY_ADMINISTRATOR`; this is neither an application PASS nor FAIL.
- Residual risks carried forward unchanged: four inherited regular-space-before-comma corpus artifacts at `LDC.T28.E0023.P070`, `LDC.T34.E0040.P016`, `LDC.T36.E0036.P021`, `LDC.T36.E0046.P016`; ten inherited v74 checkpoint source archives are not carried in this lineage; and `CONC-LDC-MIGRATION-BYPASS-01` remains a separate skipped-version/migration concurrency promotion gate.
- Physical/live/installed-PWA/true-offline/accessibility gates remain external.
- Public deployment: **NOT AUTHORIZED BY THIS PACKAGE**.
...
> **Historical/inherited boundary:** Everything below this line is predecessor or inherited-authority documentation. The v79 Stage 4A transactional repair remains active, but older package CURRENT/version/deployment wording cannot override v80.

<!-- V79_CURRENT_START -->
# INHERITED ACTIVE AUTHORITY — v2.19.79-R1B CONC-LDC-01 STAGE 4A TRANSACTIONAL INTEGRITY SUCCESSOR

- Direct predecessor: immutable `v2.19.78-R1B` SHA-256 `a1c78fff7d2893b3b38344c0da8a2cc483eb3186bc580f84b3d9b57dc3ecddcb`.
- Owner authorization: exact frozen CONC-LDC-01 Stage 4A proposal SHA-256 `ae14ae9bce3b5c1ab9075e0a3879c963ec16126fcde551333de70ad8943bbb3f`.
- Functional mutation: `index.html` only — fresh-record IndexedDB readwrite transactions and patch-only conflict handling for collection note/reorder/add/delete/rename/item-delete/item-Undo plus automatic staleness reconciliation.
- Same-record conflict rule: fail closed and preserve the newer durable value. Deleted records/parents are never recreated by stale operations; collection operations may not leave orphans.
- Exact-code engineering QA: transaction suite **17/17 PASS**; staleness-refresh suite **6/6 PASS**; pre-freeze binding verifier **171/171 PASS**; independent second-path challenge **35/35 PASS**; deliberate bypass mutants **3/3 caught**.
- `speech_model.js` remains byte-identical to the authorized v77 renderer SHA-256 `3bd067956a5275e0be7231add3b425c124fb17f3d0be548080eee10c649134f6`.
- Canonical/devotional wording, paragraph IDs/order, speaker/display/flow authority, SEARCH-V2 payload/ranking semantics, supplement content, IndexedDB schema and historical migration algorithms: **unchanged**.
- `VER-LINEAGE-LDC-01 = SEPARATE_VERSION_SIMPLIFICATION_REQUIRED`; legacy `vX.Y.Z-R1B` technical line retained.
- Managed Chromium live navigation remains environment-blocked (`ERR_BLOCKED_BY_ADMINISTRATOR`); physical/live/PWA/offline/accessibility gates remain open.
- Known inherited corpus warnings remain open and unchanged: four visible regular-space-before-comma artifacts at `LDC.T28.E0023.P070`, `LDC.T34.E0040.P016`, `LDC.T36.E0036.P021`, `LDC.T36.E0046.P016`; they are outside Stage 4A and require separate corpus authority.
- Stage 4B offline-cache retention remains a separate next LDC repair before cumulative device/public promotion.
- Public deployment: **NOT AUTHORIZED BY THIS PACKAGE**.
...
> **Historical boundary:** Everything below this line is predecessor/historical package documentation. Older CURRENT/PASS/FAIL/version wording cannot override the v79 authority index or external final-ZIP receipt.

<!-- V78_HISTORICAL_START -->
# HISTORICAL — v2.19.78-R1B DEEP FOUR-PASS STALE-GATE METADATA REPAIR

- Direct predecessor: immutable `v2.19.77-R1B` SHA-256 `69c1d171adae77aa2c7dc9a38df7f33fba95565da7834059fd7cd22dcee1cfda`.
- Trigger: user-directed deep four-pass recheck found three current physical-device gate fields still carried obsolete predecessor-candidate labels. The fields correctly asserted no device PASS, but their current-candidate designation was obsolete under v77.
- Repair scope: current report/device-gate metadata plus mechanically required release/cache/offline/search-wrapper bindings only.
- The three gates now state that v78 has **not** been physically certified and identify v73 only as the last carried candidate reference; no physical-device PASS is asserted.
- `speech_model.js` remains byte-identical to the exact authorized v77 renderer candidate SHA-256 `3bd067956a5275e0be7231add3b425c124fb17f3d0be548080eee10c649134f6`.
- Canonical/devotional wording, paragraph IDs/order, speaker semantic authority, display/flow authority payloads, SEARCH-V2 payload/ranking semantics and user-state schemas: **unchanged**.
- Renderer fixed points inherited and rechecked: 88/88 closure; 21,089 governed boundaries / zero mismatch; 20,811 RA19B; 283 compiled FAST FLOW_JOIN; 410 compiled FAST repairs; 694 successor-record hashes.
- Blocked `LDC.T29.E0003.P068` remains unchanged at SHA-256 `2bf3694d9e26921b09eb781914f4d01ce7af26feb51013d8c038572d4566df0d`.
- Live GitHub, installed-PWA persisted data, true-offline cold reopen, physical-device and accessibility gates remain external/open.
- Public deployment: **NOT AUTHORIZED BY THIS PACKAGE**.
...
> **Historical v78 boundary:** The v78 freeze-point wording below is preserved as predecessor evidence and cannot override current v79 authority.

<!-- V77_CURRENT_START -->
# CURRENT — v2.19.77-R1B AUTHORIZED RENDERER SUCCESSOR

- Direct predecessor: immutable `v2.19.76-R1B` SHA-256 `df5ebdb4fed3690def49f1b0169104bec8ad5991e19f5b4d425a45ace7939536`.
- Authorization: exact frozen renderer proposal SHA-256 `427f90ecc20d0149ca2561043c080c1dcab438f6985615a6cda2a0a7f9459961`; functional source mutation is `speech_model.js` only.
- Renderer repair: explicit `RA19B_MULTI_SOURCE / SOURCE_BACKED_CERTAIN` and `FAST_MODE_BODY0277_0854 / CERTIFIED_CORRECTION_CERTAIN` visual-boundary authority is validated and consumed before semantic same-speaker linking. Non-governed legacy boundaries retain inherited behavior.
- Corpus/devotional wording, paragraph IDs/order, speaker semantic authority, display/flow authority payloads, SEARCH-V2 payload/ranking semantics and user-state schemas: **unchanged**.
- Mandatory governed topology fixed point: 21,089 explicit boundaries = 20,811 RA19B + 278 current FAST-generation physical boundaries; original 283 compiled FAST FLOW_JOIN regression universe remains a separate gate.
- Known v76 mismatch universe: 88 = 36 FAST + 52 RA19B; successor requirement is 88/88 closure.
- Blocked `LDC.T29.E0003.P068` remains unchanged at SHA-256 `2bf3694d9e26921b09eb781914f4d01ce7af26feb51013d8c038572d4566df0d`.
- Package-local static/algorithmic certification is subject to the external final-ZIP receipt and fresh-reopen evidence. Live GitHub, installed-PWA, true offline cold reopen and physical-device/accessibility validation remain external.
- Public deployment: **NOT AUTHORIZED BY THIS PACKAGE**.
...
> **Historical boundary:** Everything below this line is predecessor/historical package documentation. Older CURRENT/PASS/FAIL/version wording cannot override the v77 authority index or external final-ZIP receipt.
