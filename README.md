...
## v2.17.17 — R5 authorized source-critical corpus successor

v2.17.17 applies the user-authorized R5 source-critical Marian corpus ledger to the immutable v2.17.16 baseline. Exactly **91 governed paragraph texts** change; all **753 stable paragraph IDs**, unit/section order and user-state identifiers are preserved. The corpus advances from **1.0.0 to 1.0.1** and the content cache advances from **mjv-content-v1 to mjv-content-v2** so installed users cannot remain pinned to predecessor text.

The release also ships a bounded 1.0.0→1.0.1 offset-migration map. Existing range highlights still prefer exact saved-text recovery; only records bound to the exact predecessor paragraph fingerprint may use the generated opcode map as a final recovery path. Selected-note anchors use the same bounded mapping when their saved selection no longer matches directly.
...
This is a corpus-content successor to v2.17.16 H.9-R1. It applies exactly **91** mutations authorized under R5 (37 Tier A / 49 Tier B / 5 Tier C) to the immutable predecessor corpus SHA-256 `63211c89125db278a971dbf1bd658d5fb9b7bd1a2128f130f930ed6783a14364`. The resulting governed corpus SHA-256 is `884b83aaf28e51d2eb6318f4f044e6d58568211f41d6df25c0ef43d4bb8560b8`.

No unit, section or paragraph identifier is added, removed or reordered: structure remains **37 units / 188 sections / 753 paragraphs**. The 662 non-target paragraphs remain text-identical. Paragraph fingerprints are regenerated only where text changes.

The runtime changes are limited to release/corpus identity, content-cache generation, the version-qualified corpus fetch needed to defeat an old controlling content cache on first upgrade, and bounded predecessor→successor highlight/note anchor migration. Backup schema remains v4 and older supported backups remain accepted with corpus-version/fingerprint differences treated as warnings before normal stable-ID reconciliation.

Physical installed-PWA update behaviour still requires a short post-deploy device smoke test; static/headless validation cannot substitute for real iPhone/iPad/Android update testing.
...
Current PWA caches are `mjv-shell-v2.17.17` and `mjv-content-v2`. The former `mjv-fonts-v1` cache remains historical only and normal `mjv-` cache cleanup removes obsolete app-scoped caches after upgrade.
...
- `mjv-shell-v2.17.17` — release-specific app shell
- `mjv-content-v2` — governed corpus 1.0.1 + predecessor→successor migration map
...
- Pure logic lives behind the `Pure` test seam. `App.selfTest()` currently contains **137 assertions**.
...
- MJV-C introduced machine backup schema v2. Current v2.17.17 uses schema v4 because reading positions are semantic records; v2.11.0/MJV-B schema-v1, v2.12.0/MJV-C schema-v2 and v2.13.0/MJV-D schema-v3 backups remain accepted and normalized during validation.
